...
def evalutate_calculation():
    global calculation
    try:
        calculation = str(eval(calculation)) # 보안상 위험한 코드 -> 실제 프로덕트 수준에서는 사용하면 안됨.
        text_result.delete(1.0, "end")
        text_result.insert(1.0, calculation)
    except:
        clear_field()
...
text_result.grid(columnspan=5, padx = 0, pady=0)
...
btn_equals = tk.Button(root, text='=', command=evalutate_calculation, width=5, font=("Arial", 14))
btn_equals.grid(row=6, column=2, padx = 0, pady=0)
btn_clear = tk.Button(root, text='C', command=clear_field, width=5, font=("Arial", 14))
btn_clear.grid(row=6, column=1, padx = 0, pady=0)
btn_point = tk.Button(root, text='.', command=lambda: add_to_calculation('.'), width=5, font=("Arial", 14))
btn_point.grid(row=6, column=3, padx = 0, pady=0)
